...
export const History = () => {
  ...
  const { loading: historyLoading, error: historyError, data: historyData} = useQuery(GET_TEST_HISTORY, {
  ...
  if (data?.getUserIdByToken.success || historyData?.getUserTestHistory.success) {
    const selectedTestData = historyData?.history.data.find((test: any) => String(test.date) === selectedDate);
    ...
    return (
      ...
      {selectedTestData && (
        <div>
          <h3>날짜: {new Date(Number(selectedTestData.date)).toLocaleDateString()}</h3>
          ...
          <ul>
            {selectedTestData.questions.map((question: any, qIndex: number) => (
              <li key={qIndex}>
                단어: {question.word} -- 사용자 응답: {question.userAnswer} -- 정답: {question.answer}
              </li>
            ))}
          </ul>
        </div>
